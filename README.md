...
## 风控参数 (live_runner v2 — richenlin 双模式设计)

改一行 `RISK_MODE = "conservative"` / `"aggressive"` 切换。

| 参数 | conservative（默认） | aggressive |
|------|---------------------|------------|
| 硬止损 | -5% | -8% |
| 追踪止损 | 6%（先盈利 3% 激活） | 10%（先盈利 5% 激活） |
| 单日熔断 | 10% | 20% |
| 冷却时间 | 8h | 4h |
| 初始保护 | z-score < 30 不买入 | 同 |

追踪止损采用 richenlin 的"激活门槛"设计：入场后需先盈利超过阈值，追踪止损才生效。避免刚入场就被微弱回撤震出。
